...
@router.get("/recipes/{recipe_uid}", response_model=Recipe)
...
@router.put("/recipes/{recipe_uid}", response_model=Recipe)
def update_recipe(recipe_uid: str, recipe: Recipe):
    return recipes_db.update(recipe_uid, recipe)


@router.delete("/recipes/{recipe_uid}", response_model=Recipe)
